...
OUTPUT_FILE = 'src/dtol/sql/dtroomscases.sql'
		...
		for line in fin:
			...
			if wallinfo[1][:5] != 'dalle':
				...
				for floor in wall['floor']:
					fout.write('insert into dt_rooms_cases_sol values (dtroomcase_id, dtfloor_id) values (%d, %d);\n' % (wall['id'], floor))
